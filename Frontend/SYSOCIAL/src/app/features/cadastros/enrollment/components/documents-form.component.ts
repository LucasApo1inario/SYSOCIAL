  ...
  template: `
    <div [formGroup]="parentForm" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-8">
      ...
      <div formArrayName="docs">
        <div *ngFor="let doc of docs.controls; let i = index" [formGroupName]="i" class="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200 flex items-center gap-6 relative group hover:border-gray-300 transition-colors">
          
          <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
            <div>
            ...
            <div>
              ...
              <div class="flex items-center">
                ...
                <button type="button" (click)="fileInput.click()" class="px-4 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-white hover:border-blue-400 hover:text-blue-600 focus:ring-2 focus:ring-blue-100 transition-all flex items-center gap-3 w-full group-hover:shadow-sm">
                  ...
                  <span class="truncate font-medium" [class.text-gray-300]="!doc.get('fileName')?.value" [class.text-gray-700]="doc.get('fileName')?.value">{{ doc.get('fileName')?.value || 'Escolher arquivo...' }}</span>
                </button>
              </div>
            </div>
          </div>

          <button type="button" (click)="remove.emit(i)" class="text-gray-400 hover:text-red-500 p-2 rounded-full hover:bg-white transition-all self-center shadow-sm border border-transparent hover:border-gray-100">
