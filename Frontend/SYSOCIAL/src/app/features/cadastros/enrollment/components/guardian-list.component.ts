    ...
    <div [formGroup]="parentForm">
      ...
      <ng-template #guardianFormTemplate let-form="form" let-i="i">
        <div [formGroup]="form" class="grid grid-cols-1 gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
              <!-- CAMPO CPF COM BUSCA AUTOMÁTICA (BLUR) -->
              <div class="md:col-span-3">
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">CPF</label>
                <input type="text" formControlName="cpf" maxlength="14" 
                       (input)="formatCPF($event)" 
                       (blur)="searchGuardian(form)"
                ...
                <p *ngIf="loadingCpf[i]" class="text-blue-500 text-xs animate-pulse font-medium">Buscando cadastro...</p>
              </div>
...
export class GuardianListComponent {
  ...
  searchGuardian(form: FormGroup) {
    ...
    const cleanCpf = cpfControl.value.replace(/\D/g, '');

    this.service.getGuardianByCpf(cleanCpf).subscribe({
      next: (data) => {
        this.loadingCpf[index] = false;
        if (data) {
          form.patchValue({
            ...
            messagePhone1: data.messagePhone1,
            messagePhone1Contact: data.messagePhone1Contact,
            messagePhone2: data.messagePhone2,
            messagePhone2Contact: data.messagePhone2Contact
          });
